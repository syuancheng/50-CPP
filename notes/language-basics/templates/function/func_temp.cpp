...
//但含义是不同的。这里的class表示T是一个类型参数，可以是任何数据类型，
//如int、float、char等，或者用户定义的struct、enum或class等自定义数据类型。
template <class T>
T Min(T a, T b) {
    return (a < b) ? a : b;
}
...
//
//为了区别类与模板参数中的类型关键字class，标准C++提出了用typename作为模板参数的类型关键字，
...
int main() {
    ...
    cout<<"2.3，3.2的最大值是："<< std::max(c,d) <<endl;
