...
int main() {
    ...
    const auto is_punctuation = [](char value) {
    ...
    const char *cp = "Hello World!!!";//字符串字面量不可修改，使用指向 const char 的指针
    ...
    //下标遍历
    for (decltype(str.size()) index = 0; index != str.size() && !is_space(str[index]); ++index)
    {
